...
# v16: the two-line frame prompt returns (v14's rounded ╭─(...)╰─
#      layout, bracketed segments), keeping nix-prompt's rounded hook
#      arrow as the prompt character: Nerd Font U+F17A9, bold, detached
#      after the ╰─ elbow, bright green on success and red on failure.
#      nix-prompt itself retires — the vendored copy is gone from this
#      file, ~/.nixprompt.bash is removed on upgrade, and starship owns
#      the prompt again.
#
# Design rules:
...
# The nix-prompt hook arrow (Nerd Font U+F17A9, TOML \U escape), kept
# by preference from the nix-prompt experiment: a detached bold mark
# after the rounded elbow (a font glyph welded to a box-drawn bar can
# never be guaranteed to align — see git history for the four attempts).
# Requires the Nerd Font build, which this script installs.
[character]
