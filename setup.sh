...
#             (bin/wallset replaces it, always as a PNG; bin/wallpaper
#             draws it)
...
configure_greeter() {
    ...
    local wall="$HOME/Pictures/wallpaper.png" bg=/usr/share/backgrounds/greeter.jpg
...
# ~/Pictures/wallpaper.png (bin/wallset records a new one there, as a
# PNG whatever it was given) and configure_greeter renders the login
# page's background from the same file. A machine with none recorded
# yet gets the repository's default (assets/wallpaper.png), copied
# once and never again: whatever the file becomes afterwards is the
# user's choice. It runs before the
...
seed_wallpaper() {
    local wall="$HOME/Pictures/wallpaper.png"
    [ -f "$wall" ] && return 0
    log "seeding the default wallpaper into ~/Pictures/wallpaper.png"
