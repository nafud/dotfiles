...
# each script owns its own lifetime. wait -n keeps the other exit
# alive: the stream ending (daemon restart) still ends the script, and
# waybar's restart-interval re-probes as before.
bar=$PPID
